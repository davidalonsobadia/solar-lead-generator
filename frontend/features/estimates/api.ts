// Estimates feature API client (client-side).
// Calls the Next.js route handlers under /api/properties/{id} and
// /api/estimates/{id}, which proxy to the backend. Never call the backend
// directly from here.
import { config } from "@/lib/config"

// Decimal columns may arrive as a JSON number or string depending on the
// backend encoder, so callers must coerce before formatting/computing.
...
export interface Estimate {
  ...
  // "complete" when consumption was derived/supplied cleanly, otherwise a
  // human-readable reason it could not be auto-filled (missing EUI benchmark,
  // owner industry or building area) — the UI surfaces this as a notice.
  status: string | null
...
export const estimatesApi = {
  /** Fetch a property's detail (fields, stakeholders, latest estimate). */
  async getProperty(propertyId: string): Promise<PropertyDetail> {
    const response = await fetch(
      config.api.endpoints.properties.byId(propertyId),
    )
    return parseJson<PropertyDetail>(response)
  },
