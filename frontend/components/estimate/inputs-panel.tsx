 ...
 *
 * Consumption is only sent when the user set it manually, so an unedited,
 * auto-filled value is left for the backend to keep (a recalculate applies only
 * the supplied fields). Incentives are always sent so the list can be cleared.
 */
...
/**
 * The Estimate screen's input panel (FE-06): an accordion form precharged from
 * the property detail that debounce-autosaves to create/recalculate the
 * estimate. Consumption arrives auto-filled from the backend (Building Area x
 * EUI); the user can override it, and the UI marks it estimated vs. manual. When
 * no EUI benchmark exists for the owner industry, a notice asks for manual entry.
 */
export function InputsPanel({ property }: InputsPanelProps) {
  ...
  // Mirror the latest state into refs after each render so the debounced save
  // (which fires later, outside render) always reads current values.
  useEffect(() => {
  ...
  // Bootstrap: when the property has no estimate yet, create one so consumption
  // arrives auto-filled and the numeric defaults populate the form. Runs once.
  useEffect(() => {
