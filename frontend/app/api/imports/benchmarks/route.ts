...
/**
 * Proxy an industry EUI benchmarks CSV upload to the backend import endpoint.
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    const summary = await apiFetch(
      config.api.endpoints.backend.imports.benchmarks,
      {
        method: "POST",
        body: forwarded,
        includeAuth: true,
    ...
    console.error("[Sunscout] Benchmarks import error:", error)
    ...
    return NextResponse.json(
      { success: false, message: "Benchmarks import failed" },
      { status: 500 },
    )
  }
}
