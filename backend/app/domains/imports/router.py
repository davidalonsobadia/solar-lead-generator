...
async def _read_csv_upload(file: UploadFile) -> str:
    """Validate an uploaded CSV and return its decoded text.

    Shared by the property and benchmark upload endpoints. Raises ``422`` when
    the file is not a ``.csv``, is empty, or is not valid UTF-8 text. A leading
    BOM is stripped transparently via ``utf-8-sig``.
    """
...
async def import_benchmarks(
    ...
    """Import the industry EUI benchmark CSV uploaded as ``multipart/form-data``.

    Upserts rows by ``(business_industry, region)`` and returns ``200`` with the
    import summary on success. Responds ``422`` when the file is missing, is not
    a ``.csv``, is not valid UTF-8 text, or is missing a required column.
    """
    content = await _read_csv_upload(file)
    ...
    db.commit()
    logger.info(
        "Benchmark import by user %s: %s", current_user.id, summary
    )
